...
function editLayoutInfo() {
	...
	for(var line in layoutInfo_map){
		...
		if(layoutInfo_map[line]["nDispId"] == HOME_DISP_ID){
			...
			if(layoutInfo_map[line]["cItemName"] == "お会計ボタン"
			|| layoutInfo_map[line]["cItemName"] == "注文履歴ボタン"
			|| layoutInfo_map[line]["cItemName"] == "店員呼び出しボタン"
			|| layoutInfo_map[line]["cItemName"] == "Laguageボタン"){
				var btn = null;
				if(layoutInfo_map[line]["cItemName"] == "お会計ボタン"){
					btn = document.getElementById("home_btn3");
				} else if(layoutInfo_map[line]["cItemName"] == "注文履歴ボタン"){
					btn = document.getElementById("top_history_btn");
				} else if(layoutInfo_map[line]["cItemName"] == "店員呼び出しボタン"){
					btn = document.getElementById("top_call_btn");
				} else if(layoutInfo_map[line]["cItemName"] == "Laguageボタン"){
				...
				// 高さ
				btn.style.height = layoutInfo_map[line]["nHeight"+MSG_CSS_LANG]+DISP_UNIT;
				// 幅
				btn.style.width = layoutInfo_map[line]["nWidth"+MSG_CSS_LANG]+DISP_UNIT;
				// Y軸
				btn.style.top = layoutInfo_map[line]["nDispPosition_Y"+MSG_CSS_LANG]+DISP_UNIT;
				// X軸
				btn.style.left = layoutInfo_map[line]["nDispPosition_X"+MSG_CSS_LANG]+DISP_UNIT;
				...
				// 透過率
				btn.style.opacity = layoutInfo_map[line]["dOpacity"];
			} else {
				// 他
				// TOP画面タグ取得
...
function goodsSelectAnimation(type,goodsNo){
	...
	// 商品画像生成
	var img = document.getElementById('ordermade_img_'+goodsNo);
		...
		for(var gData in create_dish_map[sType]){
			if(create_dish_map[sType][gData]["quantity"] >= 1){
				...
				if (gData != '92008') {
					// ハンバーグソースの場合、画像生成スキップ
					// 商品画像生成
					var img = document.getElementById('ordermade_img_'+gData);
					img.setAttribute('class','ordermade_goods_first');
					img.style.animationDelay = cnt.toString()+'s';
					ordermade_animation.appendChild(img);
					cnt = cnt.plus(0.2);
...
function changeMenubookLayout(mbCd) {
	if(mbCd == '1'){
		console.log('changeMenubookLayout mbCd = 1')
	}else if(mbCd == '2'){
		console.log('changeMenubookLayout mbCd = 2')
	}
}
